...
function ChargersPage({ establishmentId }: { establishmentId?: string }) {
  ...
  return <><section className="surface panel sems-list-page" data-testid="mvp-chargers-panel"><SectionHeader title={establishmentId ? "Carregadores dos meus locais" : "Carregadores administrados pela GoodWe"} subtitle={establishmentId ? "Monitoramento operacional sem alterar estrutura cadastral." : "Cadastro e vinculos centralizados pela GoodWe."} /><div className="sems-device-tabs"><span className="is-active">Carregador AC/DC</span><span>Gateway</span><span>Medidor</span><span>Terceiros</span></div><form><div className="sems-filter-toolbar"><button type="button" className="sems-filter-button">Filter</button><label className="sems-filter-field"><span>Local</span><select><option>Todos os locais</option></select></label><label className="sems-filter-field"><span>Status</span><select><option>Todos</option><option>Disponivel</option><option>Carregando</option><option>Offline</option></select></label><button className="sems-icon-action" aria-label="Pesquisar">⌕</button></div></form><div className="sems-status-tabs"><span className="sems-status-tab tone-info is-active"><i />Todos <b>({items.length})</b></span><span className="sems-status-tab tone-good"><i />Em operacao <b>({items.filter((item) => item.status === "available").length})</b></span><span className="sems-status-tab tone-danger"><i />Em uso <b>({items.filter((item) => item.status === "charging").length})</b></span><span className="sems-status-tab tone-muted"><i />Offline <b>({items.filter((item) => item.status === "offline").length})</b></span></div><DataTable columns={["Equipamento", "Local", "Status", "Potencia", "Energia dia", "Utilizacao", "Detalhe"]}>{items.map((charger) => <tr key={charger.id}><td><strong>{charger.id}</strong><span>{charger.internalId}</span></td><td>{state.locations.find((item) => item.id === charger.locationId)?.name}</td><td><Badge value={charger.status} /></td><td>{charger.powerKw} kW</td><td>{number(charger.todayEnergyKwh)} kWh</td><td>{charger.status === "charging" ? "68%" : "24%"}</td><td><button className="ghost-button" type="button" onClick={() => setSelectedId(charger.id)}>Selecionar</button></td></tr>)}</DataTable></section>{selected ? <section className="surface panel" data-testid="mvp-charger-detail"><SectionHeader title={`Detalhes ${selected.id}`} subtitle="Dados operacionais do equipamento selecionado." /><div className="detail-grid"><article><h3>Identificacao</h3><p>{selected.id}</p><p>{selected.internalId}</p></article><article><h3>Local</h3><p>{state.locations.find((item) => item.id === selected.locationId)?.name}</p></article><article><h3>Status</h3><p><Badge value={selected.status} /></p><p>Potencia nominal {selected.powerKw} kW</p></article><article><h3>Energia dia</h3><p>{number(selected.todayEnergyKwh)} kWh</p><p>Receita {money(selected.revenueToday)}</p></article></div></section> : null}</>;
}

function SessionRows({ items }: { items: Session[] }) {
  return <>{items.map((session) => <tr key={session.id}><td><strong>{session.id}</strong><span>{session.chargerId}</span></td><td><EntityCell title={session.driverName} subtitle={session.vehicle} /></td><td><Badge value={session.status} /></td><td>{session.durationMinutes} min</td><td>{number(session.energyKwh)} kWh</td><td>{money(session.tariffPerKwh)}/kWh</td><td>{money(session.finalAmount ?? session.consumedAmount)}</td><td><Badge value={session.payment.status} /></td></tr>)}</>;
}
...
function GenericPage({ tab }: { tab: string }) {
  ...
  if (tab === "finance" || tab === "invoices") return <section className="surface panel enterprise-page"><SectionHeader title="Financeiro" subtitle="Receita, repasses, taxas e conciliacao por estabelecimento." /><div className="kpi-grid four-cols"><KpiCard label="Receita movimentada" value={money(revenue)} help="periodo atual" accent="good" /><KpiCard label="Receita GoodWe" value={money(revenue * .06)} help="participacao" /><KpiCard label="A receber" value={money(revenue * .94)} help="estabelecimentos" /><KpiCard label="Pendencias" value="1" help="conciliacao" accent="warn" /></div><DataTable columns={["Estabelecimento", "Receita", "Participacao", "Status"]}>{state.establishments.map((item) => <tr key={item.id}><td>{item.name}</td><td>{money(revenue / state.establishments.length)}</td><td>6%</td><td><StatusPill value="Conciliado" /></td></tr>)}</DataTable></section>;
  if (tab === "operations") return <><section className="surface panel enterprise-page"><SectionHeader title="Operacao comercial" subtitle="Disponibilidade, sessoes, fila e incidentes da rede." /><div className="kpi-grid four-cols"><KpiCard label="Disponiveis" value={state.chargers.filter((item) => item.status === "available").length} help="equipamentos" accent="good" /><KpiCard label="Em uso" value={state.chargers.filter((item) => item.status === "charging").length} help="agora" accent="danger" /><KpiCard label="Fila atual" value={state.queue.filter((item) => item.status === "waiting").length} help="motoristas" accent="warn" /><KpiCard label="Offline" value={state.chargers.filter((item) => item.status === "offline").length} help="requer atencao" /></div></section><section className="surface panel decision-center"><SectionHeader title="Prioridades operacionais" subtitle="Ocorrencias ordenadas por impacto." /><div className="decision-list"><article><div className="decision-rank">01</div><div><span>Operacao · Alta</span><h3>Equipamento sem comunicacao</h3><p>Executar diagnostico remoto e acompanhar SLA.</p></div><a className="ghost-button" href="#/mvp/chargers">Investigar</a></article></div></section></>;
