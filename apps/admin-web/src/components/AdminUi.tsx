...
export function statusLabel(value: string) {
  return ({ available: "Disponivel", charging: "Em recarga", limited: "Limitado", offline: "Offline", authorized: "Autorizada", starting: "Iniciando", active: "Ativa", finished: "Finalizada", start_failed: "Falha no inicio", REQUESTED: "Solicitado", ACCEPTED: "Aceito", CONFIRMED: "Confirmado", FAILED: "Falhou", EXPIRED: "Expirado", NORMAL: "Normal", ALERT: "Alerta", CRITICAL: "Critico", STALE: "Desatualizado", UNAVAILABLE: "Indisponivel", DRAFT: "Rascunho", ACTIVE: "Vigente", RETIRED: "Encerrada", AUTHORIZED: "Autorizado", CAPTURED: "Capturado", PARTIALLY_REFUNDED: "Reembolso parcial", REFUNDED: "Reembolsado", DISPUTED: "Em disputa", PENDING: "Pendente", AVAILABLE: "Disponivel", PAID: "Liquidado", OPEN: "Aberto", ACKNOWLEDGED: "Reconhecido", IN_PROGRESS: "Em andamento", RESOLVED: "Resolvido", DEFERRED: "Adiado", REJECTED: "Rejeitado", QUEUED: "Na fila", PROCESSING: "Processando", READY: "Pronto", REVOKED: "Revogado", PAUSED: "Pausada", HIGH: "Alta", MEDIUM: "Media", LOW: "Baixa", waiting: "Aguardando", called: "Em chamada", assigned: "Admitido", no_show: "Nao compareceu", expired: "Expirado", released: "Concluido" } as Record<string, string>)[value] ?? value;
}

export function tone(value: string) {
  if (["CONFIRMED", "NORMAL", "ACTIVE", "CAPTURED", "AVAILABLE", "PAID", "RESOLVED", "READY", "assigned", "released"].includes(value)) return "good";
  if (["start_failed", "FAILED", "CRITICAL", "DISPUTED", "HIGH", "REJECTED", "no_show"].includes(value)) return "danger";
  if (["authorized", "starting", "REQUESTED", "ACCEPTED", "EXPIRED", "ALERT", "STALE", "UNAVAILABLE", "AUTHORIZED", "PARTIALLY_REFUNDED", "REFUNDED", "PENDING", "OPEN", "ACKNOWLEDGED", "IN_PROGRESS", "DEFERRED", "QUEUED", "PROCESSING", "PAUSED", "MEDIUM", "called"].includes(value)) return "warn";
