...
export function SessionPage() {
  ...
  return <>
    ...
    <InfoNotice>{session.backendManaged ? "Sessão compartilhada com a operação ChargeGrid. O pagamento usa Stripe em teste e o início depende da confirmação do carregador." : "Operação local simulada: cada 3 segundos representa 5 minutos de energia à potência exibida. A tolerância de retirada usa tempo real."}</InfoNotice>
    ...
    {session.status === CommercialSessionStatus.ENERGY_FINISHED ? <PrimaryButton onClick={beginSettlement} disabled={settling}>{settling ? "Liquidando com a Stripe…" : "Confirmar retirada e liquidar"}</PrimaryButton> : null}
