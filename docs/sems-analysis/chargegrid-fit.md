...
| --- | --- | --- | --- | --- | --- | --- | --- |
| plantas comerciais | lista, detalhe, mapa, estado e KPIs (`OBSERVED`, alta) | shell, filtros, contexto e tabs | vincular planta GoodWe a estabelecimento e perfil comercial | horários, acesso, publicação e disponibilidade | tab/área ChargeGrid dentro da planta, sem refazer o wizard técnico | `Plant`, `Establishment`; admin configura, operador lê/opera | herança exata do escopo organizacional |
...
| visão GoodWe | portfólio e escopo Distribuidor/Instalador (`OBSERVED`, alta) | dashboard de rede e drill-down | não equiparar papel observado ao papel do produto | KPIs agregados, comissão e oportunidades | rede → planta → carregador/sessão | `GOODWE_ADMIN`; projeção agregada e auditada | identidade e SSO futuros |
| visão estabelecimento/operador | Proprietário e permissões de compartilhamento (`OBSERVED`, média) | escopo por organização/planta e monitoramento/controle | decompor administração sensível da rotina | admin configura; operador atua sem tarifa/RBAC | mesmos layouts, ações condicionadas por capacidade | `ESTABLISHMENT_ADMIN`/`OPERATOR`; políticas server-side | matriz final de capacidades |
